...
> **Agent 用表结构**：AI 管理页生成 SQL 时使用的表结构以 `database/schema_for_agent.txt` 为准，该文件由脚本从当前数据库自动导出，保证与真实库一致。生成方式：在 `backend_server` 目录执行 `node scripts/export_schema_for_agent.js`。本文档为人工维护的 ER 说明，可与 `schema_for_agent.txt` 对照以保持一致。
...
- level: int (非空, GESP级别1-8)
- **category: enum('GESP','CSP','NOI','其他') (非空, 默认'GESP')** ← 新增字段
...
### OJ Submissions（OJ提交记录）

- id: int (主键, 自增)
- problem_id: int (外键, 非空, 关联OJ Problems.id)
- task_id: int (外键, 可空, 关联LearningTasks.id, NULL表示非任务内提交)
- test_attempt_id: int (外键, 可空, 关联TestAttempts.id, NULL表示非 Test 内提交)
- code: text (非空)
- language: varchar(20) (默认'cpp')
- status: enum('queued','judging','completed','error')
- verdict: varchar(50) (可空)
- total_tests: int (默认0)
- passed_tests: int (默认0)
- results: json (可空)
- error_message: text (可空)
- submit_time: datetime (默认当前时间)
- practice_duration_seconds: int (可空) 本次练习持续时间（秒），从进入页面到提交的时长
- judge_start_time: datetime (可空)
- judge_end_time: datetime (可空)
- judge_duration: int (可空, 判题耗时毫秒)
- user_id: int (可空)
- ip_address: varchar(50) (可空)
...
**公开查分 / 计划进度查询**：学生仅输入「姓名」一项。后端按 **real_name** 或 **username** 匹配（`TRIM(real_name) = 输入` 或 `username = 输入` 均可查到）。多命中时：查分取优先 real_name 再按 id；计划进度取完成度最高再 real_name 优先。展示时用 `real_name || username`。
...
- id: int (主键, 自增)
- name: varchar(200) (非空) 考试名称
- description: text (可空) 考试说明
- time_limit_minutes: int (非空) 单次作答限时（分钟）
- start_time: datetime (可空) 考试窗口开始时间（NULL 表示不限制）
- end_time: datetime (可空) 考试窗口结束时间（NULL 表示不限制）
- total_score: int (可空) 冗余满分（用于展示），可由关联题目权重汇总
- is_public: tinyint(1) (非空, 默认1) 是否公开：1=全部用户可见可参与，0=仅创建人的学生可见可参与
- created_by: int (外键, 可空, 关联Users.id) 创建者（通常为教师），用于判定「自己的学生」
- public_result_token: varchar(64) (可空) 公开查分链接 token，用于生成扫码查分 URL
- public_result_enabled: tinyint(1) (非空, 默认0) 是否开启公开查分：0=关闭，1=开启
- created_at: datetime (默认当前时间)
- updated_at: datetime (默认当前时间, 更新时自动更新)

### TestExams（Test-客观题关联）

- id: int (主键, 自增)
- test_id: int (外键, 非空, 关联Tests.id)
- exam_id: int (外键, 非空, 关联Exams.id)
- exam_order: int (默认0) 在本 Test 中的顺序
- score_weight: int (可空) 本 Exam 在 Test 中的满分（NULL 时可用 exam.total_questions 或 100）
- created_at: datetime (默认当前时间)

> 约束：UNIQUE(test_id, exam_id) 确保同一 Test 中不会重复关联同一 Exam。

### TestOjProblems（Test-OJ题关联）

- id: int (主键, 自增)
- test_id: int (外键, 非空, 关联Tests.id)
- problem_id: int (外键, 非空, 关联OJ Problems.id)
- problem_order: int (默认0) 在本 Test 中的顺序
- score_weight: int (非空, 默认100) 单题满分（用于 passed_tests/total_tests * score_weight 计算得分）
- created_at: datetime (默认当前时间)

> 约束：UNIQUE(test_id, problem_id) 确保同一 Test 中不会重复关联同一 OJ 题。

### TestAttempts（Test 参与记录）

- id: int (主键, 自增)
- test_id: int (外键, 非空, 关联Tests.id)
- user_id: int (外键, 非空, 关联Users.id)
- started_at: datetime (非空) 开始时间
- submitted_at: datetime (可空) 交卷时间（NULL 表示未交卷）
- time_limit_seconds: int (非空) 本次作答限时（秒），由 tests.time_limit_minutes * 60 冗余而来
- exam_score: int (可空) 客观题得分
- oj_score: decimal(10,2) (可空) 编程题得分
- total_score: decimal(10,2) (可空) 总分（exam_score + oj_score）
- created_at: datetime (默认当前时间)
- updated_at: datetime (默认当前时间, 更新时自动更新)

> 约束：
> - UNIQUE(test_id, user_id)：每个用户在每个 Test 仅有一条参与记录（一次机会）。
> - 排名索引：按 (test_id, total_score, submitted_at) 建复合索引，支持按总分降序、提交时间升序排序。
...
- **create_all_tables.sql**：全量建表（含 Test 相关表及对 `submissions`/`oj_submissions` 的 ALTER）。**仅适合新库**；已有库再次执行会因重复 ADD COLUMN 报错。
- **migrate_tests.sql**：仅做 Test 聚合考试相关更新，**可对已有库重复执行**：
  - 创建表：`tests`、`test_exams`、`test_oj_problems`、`test_attempts`（CREATE TABLE IF NOT EXISTS）
  - 为 `submissions`、`oj_submissions` 增加 `test_attempt_id`（仅当列不存在时执行）
- **migrate_plan_templates.sql**：学习计划模板相关表，**可对已有库重复执行**：
  - 创建表：`plan_templates`、`template_tasks`、`template_task_exams`、`template_task_oj_problems`（CREATE TABLE IF NOT EXISTS）
- 执行迁移：在 `backend_server` 目录执行 `bash database/run_migrate_tests.sh`（依赖 `.env` 中的 DB_NAME、DB_USER、DB_PASSWORD），或手动：`mysql -u user -p 数据库名 < database/migrate_tests.sql`。
