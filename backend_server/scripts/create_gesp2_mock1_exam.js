/**
 * 从 GESP 2 级客观题题库中随机抽题，组卷「GESP 2级模拟1」
...
async function main() {
  ...
  try {
    ...
    console.log(`GESP 2 级：选择题(>2选项) ${choiceIds.length} 道，判断题(2选项) ${judgeIds.length} 道`);
    ...
    const [examResult] = await connection.execute(
      `INSERT INTO exams (name, level, description, type, total_questions, bank_visible)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [examName, 2, '从GESP 2级客观题中随机抽取：前15道选择题，后10道判断题。', '模拟', 25, 0]
    );
