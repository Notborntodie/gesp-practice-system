/**
 * 录入题目 [GESP 2026年3月认证 C++ 2级 编程题2] 画画 及 10 个测试点到数据库。
 * 运行前请先执行:
 *   1) g++ -O2 -std=c++17 huahua_ref.cpp -o huahua_ref
 *   2) node gen_huahua_tests.js
 */
...
const PROBLEM = {
  ...
  description: `给你一个整数 $n$，请输出一个 $n$ 行 $n$ 列的方阵。
...
- 中间位置是 \`*\``,
  ...
  output_format: `输出 $n$ 行 $n$ 列的方阵。`,
  data_range: '保证 $1 \\le n \\le 100$。',
  time_limit: 1000,
  memory_limit: 512,
  level: 2,
...
async function main() {
  const testsPath = path.join(__dirname, 'huahua_tests.json');
  if (!fs.existsSync(testsPath)) {
    console.error('请先运行: node gen_huahua_tests.js 生成 huahua_tests.json');
    process.exit(1);
  }
