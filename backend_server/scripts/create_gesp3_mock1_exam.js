/**
 * 从 GESP 3 级客观题题库中随机抽题，组卷「GESP 3级模拟1」
...
async function main() {
  ...
  try {
    ...
    console.log(`GESP 3 级：选择题(>2选项) ${choiceIds.length} 道，判断题(2选项) ${judgeIds.length} 道`);
    ...
    const [examResult] = await connection.execute(
      `INSERT INTO exams (name, level, description, type, total_questions, bank_visible)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [examName, 3, '从GESP 3级客观题中随机抽取：前15道选择题，后10道判断题。', '模拟', 25, 0]
    );
