    ...
    methods: {
      ...
      async submitAnswers() {
        ...
        try {
          ...
          // PlanExamView 始终使用任务内提交接口
          if (!this.taskId || this.taskId.trim() === '') {
            this.alertTitle = '提交错误';
            this.alertMessage = '缺少任务ID，无法提交答案。请从任务页面进入。';
            this.showAlertDialog = true;
            console.error('❌ [PlanExamView] 缺少taskId参数，无法提交');
            this.submitting = false;
            return;
          }
          
          const submitUrl = `${this.BASE_URL}/learning-tasks/${this.taskId}/submit-exam`;
          console.log('✅ [PlanExamView] 使用任务内提交接口:', submitUrl, {
            taskId: this.taskId,
            planId: this.planId
          });
