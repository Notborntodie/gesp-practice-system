<template>
  <div class="batch-upload-page">
    ...
    <div class="editor-header">
      ...
      <h2 class="editor-title">上传练习</h2>
      <!-- Step Indicator -->
      <div class="step-indicator">
        <span :class="['step-dot', { active: step === 'input', done: step !== 'input' }]">1</span>
        <span class="step-line" :class="{ done: step !== 'input' }"></span>
        <span :class="['step-dot', { active: step === 'preview', done: step === 'exam-info' }]">2</span>
        <span class="step-line" :class="{ done: step === 'exam-info' }"></span>
        <span :class="['step-dot', { active: step === 'exam-info' }]">3</span>
      </div>
    </div>
    ...
    <div class="upload-content">
      ...
      <div v-if="step === 'preview'" class="preview-area">
        <div class="preview-header">
          <div class="preview-stats">
            ...
            <span v-if="questions.length > 0" class="stat-hint">可逐个编辑、上传图片后进入下一步</span>
          </div>
        ...
        <div class="submit-section">
          ...
          <div class="action-buttons">
            ...
            <AppButton variant="primary" :disabled="questions.length === 0" @click="goToExamInfo">
              <ArrowRight :size="16" />
              下一步：填写练习信息
            </AppButton>
          </div>
      ...
      <!-- Step 3: Exam Info -->
      <div v-if="step === 'exam-info'" class="exam-info-area">
        <div class="exam-info-header">
          <AppButton variant="ghost" size="sm" @click="step = 'preview'">
            <ArrowLeft :size="16" />
            返回修改题目
          </AppButton>
          <AppTag type="primary" size="sm">第 3 步：填写练习信息</AppTag>
        </div>

        <div class="exam-info-grid">
          <!-- 左侧：练习信息表单 -->
          <div class="exam-form-section form-section">
            <h5 class="section-title">练习信息</h5>
            <div class="form-grid">
              <AppFormField label="练习名称" :required="true">
                <AppInput v-model="examName" placeholder="如：GESP 4级 2026年3月真题" />
              </AppFormField>
              <AppFormField label="类型">
                <AppSelect v-model="examType" :options="examTypeOptions" />
              </AppFormField>
            </div>
            <div class="form-grid">
              <AppFormField label="题目来源">
                <AppSelect v-model="examCategory" :options="categoryOptions" />
              </AppFormField>
              <AppFormField v-if="examCategory === 'GESP'" label="等级">
                <AppSelect v-model.number="examLevel" :options="levelOptions" placeholder="请选择" />
              </AppFormField>
            </div>
            <AppFormField label="练习描述">
              <AppTextarea v-model="examDescription" rows="3" placeholder="可选，简要描述该练习" />
            </AppFormField>
          </div>

          <!-- 右侧：题目概览 -->
          <div class="exam-questions-section form-section">
            <h5 class="section-title">本次上传题目 ({{ questions.length }} 道)</h5>
            <div class="question-summary-list">
              <div v-for="(q, index) in questions" :key="index" class="question-summary-item">
                <span class="question-index">#{{ index + 1 }}</span>
                <span class="question-summary-text">{{ truncateText(q.question_text, 50) }}</span>
                <AppTag :type="getDifficultyTagType(q.difficulty)" size="sm">{{ difficultyLabel(q.difficulty) }}</AppTag>
              </div>
            </div>
          </div>
        ...
        <div class="submit-section">
          ...
          <div class="action-buttons">
            <AppButton variant="ghost" @click="step = 'preview'">返回修改题目</AppButton>
            <AppButton variant="primary" :disabled="!canSubmitExam || submitting" :loading="submitting" @click="submitAll">
              <Upload :size="16" />
              提交上传 ({{ questions.length }} 道题 + 练习)
            </AppButton>
          </div>
        </div>
      </div>
...
// Lucide Icons
import { ArrowLeft, ArrowRight, Eye, Upload, X, Trash2 } from 'lucide-vue-next'
...
// Step State
const step = ref<'input' | 'preview' | 'exam-info'>('input')

// Step 1 & 2: Question Input State (same as BatchUploadView)
...
// Step 3: Exam Info State
const examName = ref('')
const examCategory = ref('GESP')
const examLevel = ref<number | null>(null)
const examType = ref('真题')
...
const canSubmitExam = computed(() => {
  if (!examName.value.trim()) return false
  if (examCategory.value === 'GESP' && !examLevel.value) return false
...
function goToExamInfo() {
  // Inherit global settings as exam defaults
  examCategory.value = globalCategory.value
  examLevel.value = globalLevel.value
  step.value = 'exam-info'
}

// Image handling (same as BatchUploadView)
...
// Two-step submit: upload questions → create exam
async function submitAll() {
  if (questions.value.length === 0 || !canSubmitExam.value) return
  submitting.value = true
  submitResult.value = { type: null, message: '' }
  try {
    // Step 1: Upload questions to question bank
    const payload = questions.value.map(q => ({
      ...
      level: examCategory.value === 'GESP' ? (q.level || examLevel.value || null) : null,
      difficulty: q.difficulty,
      image_url: q.image_url || null,
      question_date: q.question_date || null,
      options: q.options || []
    }))
    const uploadRes = await axios.post(`${BASE_URL}/upload-questions-batch`, { questions: payload })
    const questionIds = uploadRes.data.results.map((r: any, i: number) => ({
      id: r.questionId,
      question_number: i + 1
    }))

    // Step 2: Create exam with the uploaded question IDs
    await axios.post(`${BASE_URL}/exams`, {
      name: examName.value,
    ...
    // Success — navigate back to exam list after short delay
    const count = questionIds.length
    successMessage.value = `${count} 道题目已上传至题库，练习《${examName.value}》已创建`
...
.exam-form-section .form-grid {
  margin-bottom: var(--space-3);
}

.exam-questions-section .section-title {
  margin-bottom: var(--space-3);
}
...
.question-summary-item .question-index {
  font-size: 10px;
  padding: 2px 6px;
}
