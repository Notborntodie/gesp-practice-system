<template>
  ...
  >
    ...
    <div class="problem-list">
      ...
      <AppEmptyState v-else-if="filteredProblems.length === 0" type="empty" description="暂无OJ题目" />
      <div
        v-else
      ...
      >
        <div class="problem-info">
          ...
          <div class="problem-meta">
            <AppTag type="info" size="sm">{{ getCategoryLabel(problem.category) }}{{ problem.level ? ` ${problem.level}级` : '' }}</AppTag>
            <AppTag type="default" size="sm">{{ problem.time_limit }}ms</AppTag>
            <AppTag type="default" size="sm">{{ problem.memory_limit }}MB</AppTag>
          </div>
        </div>
        <div class="problem-checkbox">
          <CheckCircle2 v-if="selectedProblems.includes(problem.id)" :size="24" class="checked" />
    ...
    <div class="dialog-footer">
      <div class="selected-count">
        <AppTag type="primary">已选择: {{ selectedProblems.length }}</AppTag>
      </div>
      <div class="footer-actions">
        <AppButton variant="ghost" @click="$emit('close')">取消</AppButton>
        <AppButton variant="primary" :disabled="selectedProblems.length === 0" @click="handleConfirm">
          确定
        </AppButton>
      </div>
    </div>
...
const filteredProblems = computed(() => {
  ...
  if (searchKeyword.value) {
    ...
    result = result.filter(problem => problem.title.toLowerCase().includes(keyword))
  }
