...
// Initialize form when knowledgePoint prop changes
watch(() => props.knowledgePoint, (kp) => {
  if (kp) {
    formData.value = {
      id: kp.id,
      name: kp.name || '',
      description: kp.description || '',
      category: kp.category || 'data_structure',
      level: String(kp.level || 1),
    }
    formError.value = ''
  }
}, { immediate: true })
...
async function handleSubmit() {
  ...
  try {
    const response = await axios.put(`${BASE_URL}/knowledge-points/${formData.value.id}`, {
    ...
    formError.value = error.response?.data?.message || error.message || '更新失败'
  } finally {
    loading.value = false
  }
}
